...
async function aiRESPONSE(_0x109acf, _0xf00650, _0x2728a0 = "") {
...
smd(
  ...
  async (message, input) => {
    try {
      ...
      if (!text) {
        return await message.send("*_🥲 Oga add query nau._*");
      }
...
smd(
  {
    pattern: "Power",
    desc: "Ask Power Anything. Eg facts about the University of Benin",
    category: "ai",
    // @ts-ignore
    filename: __filename,
    use: "<text>",
  ...
  async (message, input) => {
    try {
      const text = input.trim();
      if (!text) {
        return await message.send("*_Ask Power anything. Eg facts about the University of Benin_*");
      }
      ...
      if (!data || !data.status) {
        return await message.reply("*Failed to generate a response from Power.*");
      }

      const result = data.result;
      return await message.send(`POWER BARD 𝔸𝕀: ${result}`, { quoted: message });
    } catch (error) {
      await message.error(error + "\n\nCommand: bard", error, "*Failed to use Power Bard AI.*");
    }
  }
);
smd({
  pattern: "gpt4",
  category: "ai",
  desc: "Chat with Power GPT-4 AI model",
  use: "<text>",
  // @ts-ignore
  filename: __filename,
}, async (message, text, { cmdName }) => {
  if (!text) return message.reply(`*_Please provide a query_*\n*_Example ${prefix + cmdName} Who is Jordan-Joelstar_*`);
...
smd({
  pattern: "gemini",
  category: "ai",
  desc: "Chat with Power Bard AI model",
  use: "<text>",
  // @ts-ignore
  filename: __filename,
}, async (message, text, { cmdName }) => {
  if (!text) return message.reply(`*_Please provide a query_*\n*_Example ${prefix + cmdName} Who is Peter Obi?_*`);
...
smd(
  {
    ...
    // @ts-ignore
    filename: __filename,
    info: "chat with Power alexa ai!",
  },
  async (_0xe6d6e, _0x23f786) => {
    try {
      if (!_0x23f786) {
        return await _0xe6d6e.reply(
          "Hi *" + _0xe6d6e.senderName + "*, i was about swinging with Power before you summoned me. How may i help you?"
        );
      }
      const _0x55bb61 = {
        method: "POST",
...
smd(
  ...
  async (_0x1c0160, _0x482db1) => {
    try {
      return _0x1c0160.reply(await aiRESPONSE(_0x1c0160, "chat", _0x482db1));
    } catch (_0x4adf95) {
...
smd(
  ...
  async (_0x21be87, _0x17d498) => {
    try {
      ...
      return await _0x21be87.bot.sendMessage(_0x21be87.chat, {
        image: {
          url: await aiRESPONSE(_0x21be87, "dalle", _0x17d498),
        },
      ...
      await _0x21be87.error(
        _0x25b4b9 + "\n\ncommand: dalle",
        _0x25b4b9,
        "*_No RESPONSE from Dall-E Ai, Sorry!!_*"
      );
    }
  }
);
  ...
  async (_0x9bac01, _0x3700d4) => {
    try {
      ...
      return await _0x9bac01.bot.sendMessage(_0x9bac01.chat, {
        image: {
          url: await aiRESPONSE(_0x9bac01, "dalle", _0x2968fd),
        },
        caption: "*---Your DALL-E Result---*\n" + Config.caption,
      });
    } catch (_0x5d8080) {
...
smd(
  ...
  async (_0x39716c, _0xe79cfd) => {
    try {
      ...
      return await _0x39716c.bot.sendMessage(_0x39716c.chat, {
        image: {
          url: await aiRESPONSE(_0x39716c, "dalle", _0x5e79d4),
        },
        caption: "*---Your DALL-E Result---*\n" + Config.caption,
      });
    } catch (_0x110b5d) {
...
cmd({
    ...
    async(Void, citel, text) => {
        await Void.sendMessage(`${citel.chat}`, {
            // @ts-ignore
            image: log0,
            caption: `*Support : Official Power support group*\n*Group link:-https://chat.whatsapp.com/HJgOTkcWL0BBqpsMm5cT6T`,
        });
