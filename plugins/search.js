...
*/
// @ts-ignore
let Power_Md = "Power Whatsapp bot md"
...
smd({
    ...
    async(message, text) => {
 if(!text) return await message.reply('Give Me Number without + sign. Example: .bio 2557156543xx')
        var inputnumber = text.split(" ")[0]
        if (!inputnumber.includes('x')) return message.reply(`*You did not add x*\nExample: bio 2557156543xx  \n ${Config.caption}`)
        message.reply(`*Peter-power-Md is Searching for WhatsApp account in given range...* \n ${Config.caption}`)
...
smd({
    ...
    async(message, text) => {
if(!text) return await message.reply('Hello! I Am  Peter-power-Md Give Me Number without + sign. Example: .WaSearch 2557156543xx')
const inputNumber = text.split(" ")[0]
if (!inputNumber.includes('x')) return message.reply(`*You did not add x in number.*\nExample: ${prefix}WaSearch 2557156543xx  \n ${Config.caption}`)
message.reply(`*Peter-power-Md Is Searching for WhatsApp account in the given range...*\n${Config.caption}`);
function countInstances(string, word) { return string.split(word).length - 1; }
