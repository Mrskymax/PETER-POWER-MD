...
async function textToLogoGenerator(message, textProUrl, text1, text2 = "ser", serviceType = "textpro", retryOnFail = true) {
  ...
  try {
    const { textpro } = require('logomaker');
    if (text1) {
      logomakerResponse = await textpro(url, [text1, text2]);
    }
    ...
    return await message.bot.sendMessage(message.jid, {
      image: { url: logomakerResponse.image },
    ...
    try {
      ...
      let imageUrl = logomakerResponse && logomakerResponse.image ? logomakerResponse.image :
