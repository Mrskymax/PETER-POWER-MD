...
smd(
  {
    pattern: "miles",
    desc: "Ask Miles Anything. Eg facts about the University of Benin",
    category: "ai",
    filename: __filename,
    use: "<text>",
  },
  async (message, input) => {
    try {
      const text = input.trim();
      if (!text) {
        return await message.send("*_Ask anything. Eg facts about the University of Benin_*");
      }
      ...
      if (!data || !data.status) {
        return await message.reply("*Failed to generate a response from Miles.*");
      }

      const result = data.result;
      return await message.send(`𝔹𝔸ℝ𝔻 𝔸𝕀: ${result}`, { quoted: message });
    } catch (error) {
      await message.error(error + "\n\nCommand: bard", error, "*Failed to use Miles Bard AI.*");
    }
  }
);
smd({
  pattern: "gpt4",
  category: "ai",
  desc: "Chat with Miles GPT-4 AI model",
  use: "<text>",
  filename: __filename,
...
smd({
  pattern: "gemini",
  category: "ai",
  desc: "Chat with Miles Bard AI model",
  use: "<text>",
  filename: __filename,
}, async (message, text, { cmdName }) => {
...
smd(
  {
    ...
    use: "[text]",
    filename: __filename,
    info: "chat with Miles alexa ai!",
  },
  async (_0xe6d6e, _0x23f786) => {
    try {
      if (!_0x23f786) {
        return await _0xe6d6e.reply(
          "Hi *" + _0xe6d6e.senderName + "*, i was about swinging with Miles before you summoned me. How may i help you?"
        );
      }
      const _0x55bb61 = {
        method: "POST",
